    ...
    <section id="why" style={{ padding:'120px 40px', background:'var(--primary)', position:'relative', overflow:'hidden' }}>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 relative z-10"
          style={{ border:'1px solid rgba(199,166,106,0.12)', borderRadius:6, overflow:'hidden' }}>
          {items.map((item, i) => (
            <div key={i} className="group transition-colors duration-300 relative"
              style={{ padding:'48px 36px', background:'rgba(255,255,255,0.02)', borderRight: i < 3 ? '1px solid rgba(199,166,106,0.1)' : 'none' }}
              onMouseEnter={e=>(e.currentTarget.style.background='rgba(199,166,106,0.04)')}
              onMouseLeave={e=>(e.currentTarget.style.background='rgba(255,255,255,0.02)')}>
              <div className="absolute bottom-0 left-0 h-0.5 w-0 transition-all duration-300 group-hover:w-full"
                style={{ background:'linear-gradient(90deg,#C7A66A,transparent)' }} />
              <div className="w-14 h-14 flex items-center justify-center text-2xl mb-7"
                style={{ background:'rgba(199,166,106,0.1)', border:'1px solid rgba(199,166,106,0.2)', borderRadius:4 }}>
                {item.icon}
              </div>
              <span className="block text-xs mb-4" style={{ color:'#C7A66A', letterSpacing:'2px', fontFamily:"'Cormorant Garamond',serif", fontSize:14 }}>{item.num}</span>
              <h3 style={{ fontFamily:"'Noto Serif KR',serif", fontSize:19, fontWeight:700, color:'#fff', marginBottom:12, letterSpacing:'-0.3px', lineHeight:1.4 }}>
                {item.title}
              </h3>
