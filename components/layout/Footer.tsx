...
export default function Footer() {
  return (
    <footer style={{background:'#080e18',borderTop:'1px solid rgba(199,166,106,0.1)',padding:'60px 40px 32px'}}>
      <style>{`
        .footer-link{font-size:13px;color:rgba(255,255,255,0.35);text-decoration:none;transition:color 0.2s;display:block}
        .footer-link:hover{color:#E5D2A4}
        .footer-policy-link{font-size:12px;color:rgba(255,255,255,0.2);text-decoration:none;transition:color 0.2s}
        .footer-policy-link:hover{color:rgba(255,255,255,0.5)}
      `}</style>
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 pb-12" style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          <div className="md:col-span-1">
            ...
            <p className="text-sm leading-relaxed mb-5" style={{color:'rgba(255,255,255,0.35)',maxWidth:260}}>실전 투자 경험을 함께 쌓아가는<br/>부동산 교육 플랫폼</p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-7">
          ...
          <div className="flex gap-6">
            {[['#','개인정보처리방침'],['#','이용약관'],['#','사업자정보확인']].map(([href,label])=>(
              <Link key={label} href={href} className="footer-policy-link">{label}</Link>
            ))}
          </div>
        </div>
      </div>
